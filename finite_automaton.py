    ...
    def _repopulate_edge_dict(
        ...
        for (u, v) in edges:
            if u != v:
                ...
                self.edges[(u, u)] = LabeledCurvedArrow(label=edge_label, around=self[u], buffer=0.1, config=this_edge_config["label"]).rotate(angle_between(self[u].get_center(), [0, -1, 0]), axis=[0, 0, 1])
